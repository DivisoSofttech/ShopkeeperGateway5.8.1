...
class ShopkeepergatewayKafkaResourceIT {
    ...
    void setup() {
        ...
        NotificationService kafkaResource = new NotificationService(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/shopkeepergateway-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/shopkeepergateway-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
